...
bwT$alternative_tutor_strategy_id_1
ad <- Experiments %>%
  select(
        -treatment) %>%
  left_join(bwT %>%
...
# Indvidual models for each problem/experiment
```{r}
# STEP 1
...
$$pr(Next Problem Correct=1) = invLogit(\beta_{0j}+\beta_{1}Random Treatment_{i})$$ 
...
$$\beta_1 = \gamma_{10} + gamma_{11}Feature1{j} + gamma_{12}Feature2{j} …. \mu_{1j}$$
